...
def material_page3_view():
    ...
    st.markdown(
        ...
        <div class="search-box">
            <h4 style="margin-top:0px; color:#333;">🔍 자재 재고 상세 검색</h4>
        </div>
    ...
    with st.form("mat_inv_search_form"):
    ...
    if submit_btn or 'mat_page3_df' not in st.session_state:
        y = search_year if submit_btn else today.year
        m = search_month if submit_btn else today.month
        
        df = get_material_inventory_data(y, m)
        if df is not None and not df.empty:
            df = df.drop(columns=["materialinvenmanage_idx", "account_idx"], errors="ignore")
        
        st.session_state['mat_page3_df'] = df if df is not None else pd.DataFrame()
        st.session_state['mat_page3_year'] = y
        st.session_state['mat_page3_month'] = m
        st.session_state['mat_page3_item_no'] = search_item_no if submit_btn else ""
        st.session_state['mat_page3_item_name'] = search_item_name if submit_btn else ""

    df = st.session_state.get('mat_page3_df', pd.DataFrame())
    current_year = st.session_state.get('mat_page3_year', today.year)
    current_month = st.session_state.get('mat_page3_month', today.month)
    item_no_val = st.session_state.get('mat_page3_item_no', "")
    item_name_val = st.session_state.get('mat_page3_item_name', "")
